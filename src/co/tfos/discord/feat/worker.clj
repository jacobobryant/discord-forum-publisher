(ns co.tfos.discord.feat.worker
...
(defn print-usage [{:keys [biff/db]}]
  ...
  (let [n-users (nth (q db
    ...
    (log/info "There are" n-users "users."
              ...
              "by setting `:co.tfos.discord/enable-worker false` in config.edn)")))

(defn alert-new-user [{:keys [co.tfos.discord/enable-worker biff.xtdb/node]} tx]
  (doseq [_ [nil]
          :when enable-worker
